...
Дополнительно профиль теперь может задавать split tunneling по приложениям через `tun.exclude-package` и `tun.include-package`. Эти поля транслируются в Android `VpnService` и управляют обходом VPN именно на уровне приложений, а не отдельных адресов. Подробный формат с примерами описан в [android-profile-split-tunneling.md](android-profile-split-tunneling.md).
