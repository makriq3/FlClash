...
Профиль теперь также может управлять Android split tunneling по приложениям: `tun.exclude-package` исключает выбранные приложения из VPN целиком, а `tun.include-package` делает обратный режим, когда через VPN идут только указанные приложения.
